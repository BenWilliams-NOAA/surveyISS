...
#' 
#' @return Dataframe of input sample size by year, region (western - 'wgoa', central - 'cgoa', eastern - 'egoa'), 
#' species (using RACE species codes), sex (0 - combined sex, 1 - males, 2 - females; all with short description IDs in sex_desc column) and length for 
...
#' 
#' @return Dataframe of input sample size by year, region (western/central combined - 'wcgoa', eastern - 'egoa'), 
#' species (using RACE species codes), sex (0 - combined sex ,
#' 1 - males, 2 - females; all with short description IDs in sex_desc column), and length for 
#' conditional age-at-length (output saved with 'iss_caal' in file name).
#' For comparison, nominal sample size ('nss' - the number of age-length samples actually taken) is included. 
#' GOA subregion output will be denoted with w_c_egoa in output filename. Will also produce other dataframes if desired (see save_intern and save_stats argument descriptions).
#' 
...
srvy_iss_goa_wc_e_caal <- function(iters = 1, 
  ...
  # write results ----
  # input sample size
  vroom::vroom_write(out_stats$iss_caal, here::here("output", region, paste0(save, "_iss_caal_wc_egoa.csv")), delim = ",")    
  
  # if desired, write out additional statistics
  if(isTRUE(save_stats)){
    # base conditional age-at-length
    vroom::vroom_write(ogcaal, file = here::here("output", region, paste0(save, "_base_caal_wc_egoa.csv")), delim = ",")
    # bias in age & length pop'n
    vroom::vroom_write(out_stats$bias_caal, file = here::here("output", region, paste0(save, "_bias_caal_wc_egoa.csv")), delim = ",")
    # realized sample size
    vroom::vroom_write(out_stats$rss_caal, here::here("output", region, paste0(save, "_iter_rss_caal_wc_egoa.csv")), delim = ",")
  }
  
  # if desired, write out bootstrapped age & length pop'n
  if(isTRUE(save_interm)) {
    vroom::vroom_write(r_caal, here::here("output", region, paste0(save, "_resampled_caal_wc_egoa.csv")), delim = ",")
  }
