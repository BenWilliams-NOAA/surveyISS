...
apop <- function(lpop, agedat){
  ...
  # compute age pop'n for females & males
  agedat %>%
    tidytable::drop_na.() %>%
  ...
  # compute age pop'n for unsexed (og rule is if you have a year with unsexed specimen data you use all the specimen data)
  agedat %>%
    tidytable::drop_na.() %>% 
