...
#raw lengths
raw_len = bind_rows(data.frame(ebs_data$raw_length %>% 
                                 rename_all(tolower) %>% 
                                 filter(species_code %in% ebs_sp_code),surv = "EBS_SHELF"),
                  data.frame(ai_data$raw_length %>% 
                               rename_all(tolower) %>% 
                               filter(species_code %in% ai_sp_code),surv = "AI"),
                  data.frame(goa_data$raw_length %>% 
                               rename_all(tolower) %>% 
                               filter(species_code %in% goa_sp_code),surv = "GOA"))
#lengths from length table
lfreq = bind_rows(data.frame(ebs_data$length %>% 
                               rename_all(tolower) %>% 
                               filter(species_code %in% ebs_sp_code),surv = "EBS_SHELF"),
                data.frame(ai_data$length %>% 
                             rename_all(tolower) %>% 
                             filter(species_code %in% ai_sp_code),surv = "AI"),
                data.frame(goa_data$length %>% 
                             rename_all(tolower) %>% 
                             filter(species_code %in% goa_sp_code),surv = "GOA"))
#survey catch
catch = bind_rows(data.frame(ebs_data$catch %>% 
                               rename_all(tolower) %>% 
                               filter(species_code %in% ebs_sp_code),surv = "EBS_SHELF"),
                data.frame(ai_data$catch %>% 
                             rename_all(tolower) %>% 
                             filter(species_code %in% ai_sp_code),surv = "AI"),
                data.frame(goa_data$catch %>% 
                             rename_all(tolower) %>% 
                             filter(species_code %in% goa_sp_code),surv = "GOA"))
#hauls
haul = bind_rows(data.frame(ebs_data$haul %>% 
                              rename_all(tolower),surv = "EBS_SHELF"),
                data.frame(ai_data$haul %>% 
                             rename_all(tolower),surv = "AI"),
                data.frame(goa_data$haul %>% 
                             rename_all(tolower),surv = "GOA"))
...
num_ct = catch2 %>% 
  group_by(year,species_code,surv,hauljoin) %>% 
  summarise(num = sum(number_fish))
...
len_s = raw_len3 %>% 
  group_by(year,species_code,species_name,surv,sex,hauljoin) %>% 
  summarise(freq = sum(frequency))

#Annual number of lengths
ann_len = raw_len3 %>% 
  group_by(year,species_name,surv) %>% 
  summarise(freq = sum(frequency))
  ann_len = spread(ann_len,surv,freq)
  write.csv(ann_len,file = "annLen_byAreaSpecies_rep.csv",row.names = FALSE)
#3-yr average table
all_3yrAvgTotLen = ann_len %>% 
  group_by(species_name,surv) %>% 
  summarise(avg_tot_num = round(mean(freq),digits = 0))
