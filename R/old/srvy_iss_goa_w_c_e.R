...
srvy_iss_goa_w_c_e <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, r_t, yrs = NULL, 
  ...
  if(save == 'prod'){
    vroom::vroom_write(ess_age, 
                       here::here("output", region, paste0(save, "_iter_ess_ag_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, paste0(save, "_iter_ess_sz_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_age, 
                       here::here("output", region, paste0(save, "_iss_ag_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_size, 
                       here::here("output", region, paste0(save, "_iss_sz_w_c_egoa.csv")), 
                       delim = ",")
  } else{
    vroom::vroom_write(ess_age, 
                       here::here("output", region, "add_err", paste0(save, "_iter_ess_ag_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, "add_err", paste0(save, "_iter_ess_sz_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_age, 
                       here::here("output", region, "add_err", paste0(save, "_iss_ag_w_c_egoa.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_size, 
                       here::here("output", region, "add_err", paste0(save, "_iss_sz_w_c_egoa.csv")), 
                       delim = ",")
  }
