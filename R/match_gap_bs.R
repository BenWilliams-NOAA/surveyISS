...
match_gap_bs <- function(ogl, gap_lpop, thresh = 0.01, region = NULL, save = NULL){
  ...
  gap_lpop %>% 
    ...
    tidytable::select.(year, species_code, length, gap_males, gap_females, gap_unsexed, gap_total) %>% 
    tidytable::left_join.(.lpop) %>% 
    tidytable::filter.(!is.na(total)) %>% 
  ...
  # Write out results
  vroom::vroom_write(gap_match_len_n, 
                     file = here::here("output", region, paste0("gap_match_len_n_", save, ".csv")), 
                     delim = ",")
  vroom::vroom_write(gap_match_len_p, 
                     file = here::here("output", region, paste0("gap_match_len_p_", save, ".csv")),
                     delim = ",")
