...
#' 
#'
srvy_iss_ai_rebs <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, yrs = NULL, 
  ...
  oga %>% 
    tidytable::summarize.(males = sum(males),
                          ...
                          unsexed = sum(unsexed),
                          .by = c(year, age)) %>% 
    tidytable::mutate.(species_code = 3005012) -> oga_c
  ogl <- og$length
  ogl %>% 
    tidytable::summarize.(males = sum(males),
                          females = sum(females),
                          unsexed = sum(unsexed),
                          .by = c(year, length)) %>% 
    tidytable::mutate.(species_code = 3005012) -> ogl_c
