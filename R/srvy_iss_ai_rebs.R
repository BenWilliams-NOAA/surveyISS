#' replicate srvy_iss function for ai rebs stock complex
#'
...
srvy_iss_ai_rebs <- function(iters = 1, lfreq_data, specimen_data, cpue_data, strata_data, r_t, yrs = NULL, 
  ...
  if(save == 'prod'){
    vroom::vroom_write(ess_age, 
                       here::here("output", region, paste0(save, "_iter_ess_ag_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, paste0(save, "_iter_ess_sz_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_age, 
                       here::here("output", region, paste0(save, "_iss_ag_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_size, 
                       here::here("output", region, paste0(save, "_iss_sz_rebs.csv")), 
                       delim = ",")
  } else {
    vroom::vroom_write(ess_age, 
                       here::here("output", region, "add_err", paste0(save, "_iter_ess_ag_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(ess_size, 
                       here::here("output", region, "add_err", paste0(save, "_iter_ess_sz_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_age, 
                       here::here("output", region, "add_err", paste0(save, "_iss_ag_rebs.csv")), 
                       delim = ",")
    vroom::vroom_write(iss_size, 
                       here::here("output", region, "add_err", paste0(save, "_iss_sz_rebs.csv")), 
                       delim = ",")
  }
}