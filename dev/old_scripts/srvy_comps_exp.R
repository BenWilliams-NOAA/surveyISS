...
srvy_comps_exp <- function(lfreq_data, specimen_data, cpue_data, strata_data, r_t, yrs, 
  ...
  al_variab(.agedat) -> .agedat_al
  
  # add ageing error ----
  age_error(.agedat, r_t) -> .agedat_ae
  
  # with age-length and ageing error ----
  age_error(.agedat_al, r_t) -> .agedat_al_ae
  
  # age population ----
  apop(.lpop, .agedat) -> .apop
  apop(.lpop, .agedat_al) -> .apop_al
  apop(.lpop, .agedat_ae) -> .apop_ae
  apop(.lpop, .agedat_al_ae) -> .apop_al_ae
  
  list(age = .apop, age_al = .apop_al, age_ae = .apop_ae, age_al_ae = .apop_al_ae, length = .lpop)
