...
gap_old_ac <- vroom::vroom(here::here('dev', 'gapindex', 'data', 'old_bts_ac_goapop.csv')) %>% 
  dplyr::rename_all(tolower) %>% 
  tidytable::filter(age > 0) %>% 
  tidytable::summarise(agepop_old = sum(population_count), .by = c(year, species_code, age))

gap_old_lc <- vroom::vroom(here::here('dev', 'gapindex', 'data', 'old_bts_lc_goapop.csv')) %>% 
  dplyr::rename_all(tolower) %>% 
  tidytable::filter(length_mm > 0) %>% 
  tidytable::summarise(abund_old = sum(population_count), .by = c(year, species_code, length_mm)) %>% 
  tidytable::rename(length = 'length_mm')


gap_old2_ac <- vroom::vroom(here::here('dev', 'gapindex', 'data', 'old2_bts_ac_goapop.csv')) %>% 
  dplyr::rename_all(tolower) %>% 
  tidytable::filter(age > 0) %>% 
  tidytable::rename(year = 'survey_year') %>% 
  tidytable::summarise(agepop_old2 = sum(agepop), .by = c(year, species_code, age))
...
# gapindex to previous year's data
gap_ac %>% 
  tidytable::left_join(gap_old_ac) %>% 
  tidytable::mutate(diff = agepop_gapindex - agepop_old) %>% 
  print(n = 200)
# matches - but might be the new table
  
gap_ac %>% 
  tidytable::left_join(gap_old2_ac) %>% 
  tidytable::mutate(diff = agepop_gapindex - agepop_old2) %>% 
  print(n = 200)
# does not match
